...
const Modal = ({ isOpen, onClose }) => {
    return (
        <CSSTransition in={isOpen} timeout={1000} classNames="unfold">
...
const About = () => {
    ...
    const getCommands = (setIsBioOpen) => {
        return {
    ...
    const commands = getCommands(setIsBioOpen);

    return (
        <Layout commands={commands}
                welcomeMessage="Yoou've made it to the About page. Enter command 'ls' to find out more. Use 'cd' to go back to main">
            <CSSTransition nodeRef={nodeRef} in={isBioOpen} timeout={1000} classNames="unfold" unmountOnExit>
                <Modal ref={nodeRef} isOpen={isBioOpen} onClose={closeModal} />
            </CSSTransition>
            {/* other components in Layout */}
        </Layout>
    );
}

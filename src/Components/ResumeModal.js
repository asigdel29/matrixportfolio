...
`;
        ...
        if (isOpen) {
            intervalId = setInterval(() => {
                setVisibleText(prevText => {
                    const nextChar = resumeText[typingIndex];
    ...
    return (
        <div className={isOpen ? 'modal open' : 'modal'} style={{ backgroundColor: '#000000a1' }}>
            <div className="modal-content">
                <span className="close" onClick={onClose} style={{ color: '#00FF00' }}>
                    &times;
                </span>
                <div dangerouslySetInnerHTML={{ __html: visibleText }} style={{ color: '#00FF00' }}></div>
            </div>
        </div>
    );
};
